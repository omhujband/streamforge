    ...
    for (let i = 1; i < lines.length; i++) {
      ...
      if (values.length === headers.length) {
        ...
        headers.forEach((header, index) => {
          ...
          // Backwards compatible casting list including new RPA dataset fields
          if (['employee_count', 'annual_revenue_usd', 'customer_count', 'founded_year', 'robots_deployed', 'budget_usd', 'annual_savings_usd', 'employee_hours_saved'].includes(header)) {
            rowObject[header] = parseInt(val, 10) || 0;
          } else if (header === 'market_share_percent' || header === 'roi_percent') {
    ...
    try {
      ...
      setInterval(() => {
        ...
        for (let i = 0; i < batchSize; i++) {
          ...
          // Check if dataset matches the official RPA database schema or old metadata
          if ('annual_savings_usd' in row) {
            ...
            if (isAnomaly) {
              ...
              row.employee_hours_saved += Math.floor(randomRange(-500, 500));
              row.roi_percent = parseFloat((row.roi_percent + randomRange(-5, 5)).toFixed(2));
            } else {
              row.annual_savings_usd += Math.floor(randomRange(-500, 1000));
              row.robots_deployed += Math.floor(randomRange(-1, 2));
              row.employee_hours_saved += Math.floor(randomRange(-10, 30));
            }
